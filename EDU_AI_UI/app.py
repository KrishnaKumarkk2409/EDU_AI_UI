...
# Fetch events from calendarData table
def fetch_calendar_events(year, month):
    conn = get_db_connection()
    cursor = conn.cursor(dictionary=True)
    query = """
        SELECT DATE_FORMAT(date, '%Y-%m-%d') as date, event, content 
        FROM calendarData 
...
def generate_month_calendar(year, month):
    # Get the number of days in the month and the first day's weekday (0 = Monday, 6 = Sunday)
    first_day_weekday, num_days = monthrange(year, month)
    
    # Adjust first_day_weekday to make Sunday = 0, Monday = 1, ..., Saturday = 6
    first_day_weekday = (first_day_weekday + 1) % 7
    
    # Create weeks list
    weeks = []
    current_week = [0] * first_day_weekday  # Fill with zeros before the 1st
    ...
    # Fill the last week with zeros if needed
    if current_week:
...
def calendar(year=None, month=None):
    ...
    # Generate month days
    month_days = generate_month_calendar(year, month)
    
    # Fetch events for the month
    events = fetch_calendar_events(year, month)
